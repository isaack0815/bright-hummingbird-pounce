...
serve(async (req) => {
  ...
  try {
    ...
    const { rosterId, entries } = await req.json();
    if (!rosterId || !entries) {
        return new Response(JSON.stringify({ error: 'Roster ID and entries are required' }), { status: 400 });
    }

    // Delete existing entries for this roster
    const { error: deleteError } = await supabase.from('duty_roster_entries').delete().eq('roster_id', rosterId);
