...
export function AssignExternalOrderDialog({ order, settings, open, onOpenChange }: AssignExternalOrderDialogProps) {
  ...
  const pdfMutation = useMutation({
    mutationFn: async (updatedOrder: FreightOrder) => {
      ...
      if (existingFiles && existingFiles.length > 0) {
        const latestFile = existingFiles.sort((a, b) => b.id - a.id)[0];
        const versionMatch = latestFile.file_name.match(/_v(\d+)\.pdf$/);
        if (versionMatch) {
          newVersion = parseInt(versionMatch[1], 10) + 1;
        } else {
          newVersion = 2; // If first version had no number
        }
        await supabase.from('order_files').update({ is_archived: true }).eq('id', latestFile.id);
      }
      
      const sanitizedOrderNumber = updatedOrder.order_number.replace(/\//g, '_');
      const fileName = `Transportauftrag_${sanitizedOrderNumber}_v${newVersion}.pdf`;
      const filePath = `${updatedOrder.id}/${uuidv4()}-${fileName}`;

      const pdfBlob = generateExternalOrderPDF(updatedOrder, settings, newVersion);
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (updatedOrder) => {
        showSuccess("Externe Vergabe erfolgreich aktualisiert!");
        queryClient.setQueryData(['freightOrder', String(order?.id)], updatedOrder);
        queryClient.invalidateQueries({ queryKey: ['freightOrders'] });
        pdfMutation.mutate(updatedOrder as FreightOrder);
        setIsEditing(false);
    ...
    onError: (err: any) => {
        addError(err, 'API');
        showError(err.data?.error || "Fehler beim Aktualisieren.");
    }
  });
    ...
    if (option && !option.__isNew__) {
      ...
      if (carrier) {
        form.reset({
          ...
          transporter_dimensions: carrier.transporter_dimensions || '',
  ...
  const renderContent = () => {
    if (!isAssigned || isEditing) {
      return (
        <Form id="assign-external-form" onSubmit={form.handleSubmit((v) => isEditing ? updateMutation.mutate(v) : assignMutation.mutate(v))}>
      ...
      <Modal.Footer>
        {isAssigned && !isEditing && (
          <>
            ...
            <Button variant="secondary" onClick={() => setIsEditing(true)}><Edit className="me-2 h-4 w-4" /> Bearbeiten</Button>
        ...
        {isEditing && (
          <>
            ...
            <Button type="submit" form="assign-external-form" disabled={updateMutation.isPending}>
              {updateMutation.isPending ? 'Wird gespeichert...' : 'Änderungen speichern & PDF aktualisieren'}
            </Button>
          </>
        )}
        {!isAssigned && (
