...
export const RosterGrid = ({ rosterId }: { rosterId: number }) => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.functions.invoke('update-roster-entries', { body: { rosterId, entries } });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
        ...
        const { error } = await supabase.functions.invoke('delete-roster', { body: { rosterId } });
        if (error) throw error;
    },
    onSuccess: () => {
